...
require(cydar)
cd <- readRDS(file.path("../../refdata", "Cytobank_44185_raw.rds"))
out <- readRDS(file.path("../../refdata", "Cytobank_44185_counts.rds"))

# Setting up the design matrix.
patient <- sub(".*_H([0-9]+)\\..*", "\\1", colnames(out$counts))
treatment <- sub(".*NoDrug_([^_]+)_.*", "\\1", colnames(out$counts))
