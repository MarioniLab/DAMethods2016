...
We also compared edgeR to the Mann-Whitney test which is often used to detect differential proportions in flow cytometry data \cite{watson1992significance}.
The Mann-Whitney test routinely yields larger $p$-values than edgeR for the same hyperspheres (Figure~\ref{fig:testtest}B), due to the loss of power from using ranks with small sample sizes.
Thus, edgeR is more sensitive than the Mann-Whitney test while still retaining its specificity.
This suggests that edgeR is an appropriate statistical method for detecting differential abundance in mass cytometry data.
...
To measure the observed spatial FDR across the detected hyperspheres, we performed PCA on the hypersphere positions and took the first two components to define a 2D space (Figure~\suppfigfdr{}).
...
The Supplementary Materials is a single PDF file that consists of Sections~1-\suppannotate{} and contains Figures~S1-\suppfigclusterreal{} and Table~S1.
