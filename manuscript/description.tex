...
Briefly, antibodies specific to markers of interest are conjugated to heavy metal isotopes; a population of cells is stained with these antibodies; the suspension of stained cells is passed through a nebulizer; single-cell droplets are vaporized to release and ionize the metals; and the quantity of metal ions in each cell is measured by time-of-flight mass spectrometry.
...
Bespoke computational methods are required to extract meaningful biological information from mass cytometry data. 
To this end, a number of software tools have been developed, e.g., SPADE \cite{qiu2011extracting}, X-shift \cite{samusik2016automated}.
These methods have focused on clustering cells into biologically relevant subpopulations based on the ``intensity'' of each marker (i.e., the signal of the corresponding metal in the mass spectrometer) and quantifying the abundance of each subpopulation in the total cell pool.
This exploits the large number of available markers in mass cytometry data to effectively distinguish subpopulations.
...
Detection of these differentially abundant (DA) subpopulations is useful as they may provide insights into the cause or effect of the biological differences between conditions.
From a statistical perspective, it is also more straightforward to compare abundances between samples for the same subpopulation, rather than comparing abundances between subpopulations.
In the former, any subpopulation-specific biases are expected to cancel out and can be ignored to simplify the downstream analysis.
...
Of the methods that do exist, the typical strategy involves clustering cells into empirical subpopulations, and then checking each cluster for differential abundance \cite{anchang2016visualization,bruggner2014automated}.
...
(Conversely, relying only on well-separated clusters will result in loss of power to resolve subpopulations that are subtly different.)
Cluster formation is also dependent on the choice of algorithm \cite{datta2003comparisons,wiwie2015comparing}, which makes it difficult to determine the reliability of the clusters.
...
Cells from multiple samples are counted into high-dimensional subspaces, and counts for each subspace are tested for significant differences between conditions.
