...
% The problem is that you'd need to using low spacings to avoid strong edge effects.
...
% This is not computationally feasible as the assignment operation per cell would be a function of the number of possible grid points.
%
% Nonetheless, let's be optimistic and assume that we have a hypothetical shortcut where each cell only needs to consider neighbouring grid points.
% Further assume that, on each dimension, each cell could get assigned to two grid points (i.e., hyperspheres are arranged in a brickwork pattern).
% This still makes for a million possible assignments per cell, so the optimal time complexity would be something like N*1e6 where N is the number of cells.
% In contrast, the current complexity is close to N*sqrt(N); you'd have to have a huge experiment before this becomes slower than the above.
%
...
% This might be more statistically complicated, but it avoids the computational effort of setting up and optimizing the grid.
