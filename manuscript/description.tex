...
\revised{One specific example is that of} large-scale immunophenotyping efforts by both human and mouse phenotyping consortia \cite{maecker2012standardizing,brown2012international,finak2016standardizing}\revised{, which rely on the identification of subpopulations that change in abundance between groups.}
...
\revised{Furthermore, careful calculation of the median-based positions is required to handle substantial differences in the numbers of cells between samples, as described in Section~\suppweightmedian{} of the Supplementary Materials.}
...
We ignore hypersphere-specific biases (e.g., due to \revised{misassignment of cells that have been over-/understained}) as these are expected to cancel out when comparing counts from the same hypersphere.
