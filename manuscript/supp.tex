...
\begin{figure}[tbp]
    ...
    \end{center}
    \caption{\revised{Calculation of the observed spatial FDR from hypersphere positions in a simulation based on the Oct4-GFP time course.
        Detected hyperspheres were defined using the na\"ive BH method to nominally control the FDR at 5\%.
        Each detected hypersphere was allocated to a non-overlapping hypercubic partition based on its position.
        In each non-empty partition, the proportion of assigned hyperspheres that were false positives was calculated.
        The average proportion across partitions was defined as the observed spatial FDR.
        (If all partitions only contain true or false positives, the spatial FDR is simply the proportion of false positive partitions.)
        The PCA plot above demonstrates this process in two dimensions, where each partition is represented by a pixel of width 0.5.}
        ...
        \revised{In this example, the observed spatial FDR is over 90\%.} 
    }
\end{figure}
