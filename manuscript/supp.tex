...
We verify the choice of formulation of $r$ by examining the distance between each cell $x$ and its neighbours in the first sample of each time course.
...
In contrast, cells in other subspaces will cease to be counted into the hypersphere as they become separated from $x$ on other markers, i.e., specificity of counting is improved.
...
This means that around 10 cells from the subspace will be counted into each hypersphere when $r_0\sqrt{2}=0.5$.
...
We note that the use of a smaller $r_0$ leads to a large drop in the number of DA hyperspheres, though for this data set, a smaller $r_0$ is not advisable since it means that the majority of hyperspheres will contain only one cell (Figure~\ref{fig:nvd}).
...
Identification of offending subpopulations can be done before the DA analysis based on existing knowledge, or afterwards based on the top DA hyperspheres.
...
Note that this is only necessary for differential subpopulations with many cells, as changes in small subpopulations will not have a major effect on the total count.
...
Let each null hypothesis $i$ be associated with a $p$-value $p_i$ and a frequency weight $f_i$.
Assume that there are $n$ null hypotheses, ordered such that $p_1 < p_2 < ... < p_n$.
The BH method can be trivially extended to incorporate these frequency weights by rejecting any null hypothesis where the $p$-value is less than
\[
    T = \max\left\{ p_i : p_i \le \alpha \frac{\sum_{i'=1}^{i} f_i}{\sum_{i'=1}^{n} f_i} \right\} \;.
\]
This is equivalent to treating the weighted hypothesis $i$ as a collection of $f_i$ unweighted hypotheses, and applying the standard BH method to those unweighted hypotheses. 
...
While the collection of hypotheses are completely dependent, this should not compromise FDR control as the BH method is robust to dependencies \cite{reiner2003identifying,kim2008effects}.

To control the spatial FDR, we apply the BH method with frequency weights to the hyperspheres.
...
It is also accurate for small partitions, as strong correlations betwen hyperspheres in the same partition mean that the expected set of hyperspheres will have a similar (frequency-weighted) distribution of $p$-values as any instance of the sampled set.
...
%\textbeta-Catenin and I\textkappa{}B\textalpha{} levels were lower than in MEFs.
    \item An small subset of MEFs in Figure~\ref{fig:oct4} exhibited very high Cd140a expression.
This represents OSKM non-expressing cells that reverted to a MEF-like endpoint state after doxycycline withdrawal.
    ...
    \item The Nanog\hi{} trajectory was identified as being intermediate for Nanog.
This apparent contradiction is because Nanog levels increase across the trajectory.
