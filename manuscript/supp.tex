...
The following text is largely paraphrased from Lun \textit{et al.} \cite{lun2016delicious}, with some minor adjustments. 
...
\]
where $\beta_{hk}$ is the hypersphere-specific value of coefficient $k$, $x_{jk}$ represents the sample-specific predictor for $k$, and $o_{hj}$ is the hypersphere- and sample-specific offset.
...
A separate mean-dependent trend is fitted to the raw QL dispersions to $\mu_h$ for all hyperspheres, and empirical Bayes shrinkage is performed to squeeze the raw estimates towards this trend \cite{lund2012detecting}.
