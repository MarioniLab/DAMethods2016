...
\begin{enumerate}
    ...
    \item Define a batch-specific adjustment function $a_{mb}(.)$ that takes $Q_{mjscb}$ as input and returns $Q'_{mjscb}$.
...
Unlike quantile normalization, a range-based normalization approach avoids assuming that all differences between batches are technical.
...
\begin{figure}[tbp]
    ...
    \end{center}
    \caption{\revised{Observed type I error rates for the simulation of intensity shifts, using either the default hypersphere radius ($r_0\sqrt{2}=0.5$) or an expanded radius designed to accommodate the estimated shift.
