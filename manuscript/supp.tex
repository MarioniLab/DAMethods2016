...
\revised{The negative binomial distribution is well-suited to this application, as it is supported over all non-negative integers and can accurately model both small and large cell counts.
...
\revised{Estimation of $\beta_{hg}$ for each $h$ and $g$ is performed by fitting the GLM to the count data for each hypersphere.}
...
\revised{Thus, it is essential that the data set contains some level of replication for dispersion estimation (though this should already be standard procedure in any good experimental design).
...
\revised{This increases the accuracy of the model by accounting for empirical mean-variance relationships.
...
\revised{These improve the accuracy of the model by accounting for hypersphere-specific variability above/below that modelled by the trended NB dispersion.
...
% The t-SNE of the hypersphere locations can be interpreted as a smoothed t-SNE of the cells.
% This is based on the fact that the locations are medians of a hyperspherical kernel.
...
\end{enumerate}
\end{color}
\revised{Hyperspheres are constructed using only the non-shifting markers in Set~\ref{set:unshift}.}
Within each hypersphere, the median intensity of a \revised{shifting marker in Set~\ref{set:shift}} is computed across all cells in each sample.
...
The precision of the sample median is approximately proportional to the number of values from which it is computed \cite{rider1960variance} -- this ensures that a \revised{median calculated from a few cells is downweighted to reflect its} instability.

Once the linear model is fitted to the medians, a variance estimate is obtained for each hypersphere.
Note that this represents the variability of the median intensities across replicates, \textit{not} the variability of the cell intensities within each sample, \textit{nor} the variability \revised{of the cell counts across samples}.
...
In this manner, each hypersphere is tested for shifts in intensity \revised{for each marker in Set~\ref{set:shift}}.
The spatial FDR is then controlled using these $p$-values.
\revised{This is reasonable as the only purpose of the spatial FDR is to account for variations in the density of hyperspheres.
Thus, the procedure can be applied to any hypersphere-level $p$-values, be they for intensity shifts or changes in abundance.}

It should be stressed that any markers to be tested for shifting \revised{(i.e., in Set~\ref{set:shift})} should \textit{not} be used when assigning cells to hyperspheres \revised{(Set~\ref{set:unshift} only)}.
Consider a marker $X$ containing potential shifts in intensity \revised{between conditions}.
...
%    \end{center}
%    \caption{A PCA plot of the positions of the detected hyperspheres in the FDR control simulation based on the \textit{Oct4}-GFP time course.
...
\begin{figure}[p]
    \begin{center}
        \includegraphics[width=\textwidth,trim=10mm 45mm 0mm 0mm,clip]{pics/Figure_BMMC.pdf}
    \end{center}
