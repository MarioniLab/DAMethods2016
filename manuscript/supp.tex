...
Using a larger $r_0$ increases the number of detected hyperspheres (Supplementary Table~\ref{tab:param}).
...
Subspaces can be tested that would previously have been filtered out, while larger counts also increase power to reject the null.
...
This loss of power is due to a decrease in the volume and count size, whereby most hyperspheres contain only one cell (Supplementary Figure~\ref{fig:nvd}).
...
\begin{figure}[p]
    ...
    \end{center}
    \caption{The effect of scaling the hypersphere radius (based on the choice of $r_0\sqrt{2}$) on the cell count for each hypersphere in the Oct4-GFP time course.
